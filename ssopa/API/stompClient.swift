...
public class StompClientLib: NSObject, SRWebSocketDelegate {
    ...
    public func subscribeToDestination(destination: String, ackMode: StompAckMode) {
        ...
        connectionHeaders![StompCommands.commandHeaderDestination] = destination
        connectionHeaders![StompCommands.commandHeaderAck] = ack
        connectionHeaders![StompCommands.commandHeaderDestinationId] = destination
...
//        }
        self.sendFrame(command: StompCommands.commandSubscribe, header: connectionHeaders, body: nil)
    }
